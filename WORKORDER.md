...
### 6-3. 소셜 로그인 [⚠ 서버 작업이 선행되어야 한다]

`login_screen.dart:76`이 "소셜 로그인은 준비 중입니다"로 막혀 있다.

**서버에 소셜 로그인 지원이 전혀 없다** — `auth.ts`에 OAuth 경로가 없고
`users` 테이블에 provider 컬럼도 없다(허브 확인, 2026-08-20). 앱만으로는
붙일 수 없다. 서버 TASKS.md에 백엔드 신설을 지시해 뒀다.

- [ ] **어떤 공급자를 붙일지 먼저 정한다** — Google / Apple / 카카오.
      국내 사용자 기준이면 카카오, 글로벌 네트워킹 앱 성격이면 Google이 먼저다
- [ ] ⚠ **iOS 규정**: 타사 소셜 로그인을 하나라도 넣으면 **Sign in with Apple을
      함께 제공해야 심사를 통과한다.** 지금은 소셜이 아예 없어 문제가 없지만,
      추가하는 순간 Apple 로그인이 세트로 따라온다. 공수 산정에 반영할 것
- [ ] 서버와 스펙 합의(토큰 검증 주체·계정 병합 규칙) 후 착수

### 6-4. 포인트 미니게임 [기획 대기 — 대표 확인]

제휴 탭에 "포인트 미니게임 / 게임하고 포인트 적립하기 / 오픈 예정" 카드가
자리만 잡혀 있다. **무슨 게임인지, 적립 규칙이 무엇인지 기획이 없다.**
구현할 수 없으므로 **인박스에 판단 요청으로 올리고 대기**한다.
...
| **포인트 미니게임 기획** | 6-4 — 무엇을 만들지 정해져야 착수 가능 |
| 소셜 로그인 공급자 선택 | 6-3 — Google/Apple/카카오 중 어느 것 |
